...
namespace SP
{
    ...
    public partial class SegundoParcial : Form
    {
        ...
        private static string ObtenerListadoFrutas()
        {
            ...
            SqlConnection connection = new SqlConnection(Properties.Settings.Default.Frutas);
            string datos = "";
            ...
            try
            {
                ...
                command.CommandText = "SELECT * FROM [sp_lab_II].dbo.frutas";
                SqlDataReader reader = command.ExecuteReader();


                while (reader.Read())
                { 
                    datos += reader["nombre"].ToString() + reader["peso"].ToString() + reader["precio"].ToString() + "\n";
                }
            }
            catch (Exception e)
            {
        ...
        private static bool AgregarFrutas(SegundoParcial frm)
        {
            ...
            try
            {
                ...
                foreach (Manzana item in frm.c_manzanas.Elementos)
                {
                    command.CommandText = "INSERT INTO [sp_lab_II].dbo.frutas (nombre, peso, precio) values" +
                                     "('" + item.Nombre + "', '" + item.Peso.ToString() + "', '" + frm.c_manzanas.PrecioUnitario.ToString() + "')";
                }
                foreach (Banana item in frm.c_bananas.Elementos)
                {
                    command.CommandText = "INSERT INTO [sp_lab_II].dbo.frutas (nombre, peso, precio) values" +
                                     "('" + item.Nombre + "', '" + item.Peso.ToString() + "', '" + frm.c_bananas.PrecioUnitario.ToString() + "')";
                }
                foreach (Durazno item in frm.c_duraznos.Elementos)
                {
                    command.CommandText = "INSERT INTO [sp_lab_II].dbo.frutas (nombre, peso, precio) values" +
                                     "('" + item.Nombre + "', '" + item.Peso.ToString() + "', '" + frm.c_duraznos.PrecioUnitario.ToString() + "')";
                }
